...
// The calculators are parameterized in calcium_type; the literals below are spelled in that type so that
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorConstructorRelative) {
    ...
    const auto decay_amount = RandomFactory::get_random_double(std::nextafter(utility::as<calcium_type>(0.0), utility::as<calcium_type>(1.0)), utility::as<calcium_type>(1.0), mt);
    ...
    ASSERT_NO_THROW(RelativeDecayCalciumCalculator cc1(utility::as<calcium_type>(0.1), decay_interval_1)) << 1.0 << ' ' << 1000;
    ASSERT_NO_THROW(RelativeDecayCalciumCalculator cc2(utility::as<calcium_type>(0.1), decay_interval_2)) << 1.0 << ' ' << decay_step;
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorConstructorAbsolute) {
    ...
    ASSERT_NO_THROW(AbsoluteDecayCalciumCalculator cc1(utility::as<calcium_type>(1.0), decay_interval_1)) << 1.0 << ' ' << 1000;
    ASSERT_NO_THROW(AbsoluteDecayCalciumCalculator cc2(utility::as<calcium_type>(1.0), decay_interval_2)) << 1.0 << ' ' << decay_step;
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorConstructorRelativeException) {
    ...
    const auto decay_amount_low = RandomFactory::get_random_double(utility::as<calcium_type>(-1000.0), std::nextafter(utility::as<calcium_type>(0.0), utility::as<calcium_type>(-1.0)), mt);
    const auto decay_amount_high = RandomFactory::get_random_double(utility::as<calcium_type>(1.0), utility::as<calcium_type>(1000.0), mt);
    ...
    ASSERT_THROW_NO_PRINT_MSG(RelativeDecayCalciumCalculator cc2(utility::as<calcium_type>(1.0), decay_interval_2), RelearnException, fmt::format("{} {}", 0.0, 1000));
    ASSERT_THROW_NO_PRINT_MSG(RelativeDecayCalciumCalculator cc3(utility::as<calcium_type>(0.5), decay_interval_1), RelearnException, fmt::format("{} {}", 0.5, 0));
}
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorConstructorAbsoluteException) {
    ...
    const auto decay_amount = RandomFactory::get_random_double(utility::as<calcium_type>(-1000.0), std::nextafter(utility::as<calcium_type>(0.0), utility::as<calcium_type>(-1.0)), mt);
    ...
    ASSERT_THROW_NO_PRINT_MSG(AbsoluteDecayCalciumCalculator cc1(utility::as<calcium_type>(0.5), decay_interval_1), RelearnException, fmt::format("{} {}", 0.5, 0));
    ASSERT_THROW_NO_PRINT_MSG(AbsoluteDecayCalciumCalculator cc1(utility::as<calcium_type>(0.0), decay_interval_2), RelearnException, fmt::format("{} {}", 0.0, 100));
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorConstructurGetter) {
    ...
    const auto decay_amount_relative = RandomFactory::get_random_double(utility::as<calcium_type>(0.0), std::nextafter(utility::as<calcium_type>(1.0), utility::as<calcium_type>(0.0)), mt);
    const auto decay_amount_absolute = RandomFactory::get_random_double(std::nextafter(utility::as<calcium_type>(0.0), utility::as<calcium_type>(1.0)), utility::as<calcium_type>(1000.0), mt);
    const auto decay_step = RandomFactory::get_random_integer<RelearnTypes::step_type>(0, 10000000, mt);
    ...
    const auto cc2 = RelativeDecayCalciumCalculator(static_cast<RelearnTypes::calcium_type>(decay_amount_relative), decay_interval);
    const auto cc3 = AbsoluteDecayCalciumCalculator(static_cast<RelearnTypes::calcium_type>(decay_amount_absolute), decay_interval);

    ASSERT_NEAR_EPS(cc2.get_decay_amount(), decay_amount_relative);
    ASSERT_NEAR_EPS(cc3.get_decay_amount(), decay_amount_absolute);
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorGetterSetter) {
    ...
    auto cc2 = RelativeDecayCalciumCalculator(static_cast<RelearnTypes::calcium_type>(decay_amount_relative), decay_interval);
    auto cc3 = AbsoluteDecayCalciumCalculator(static_cast<RelearnTypes::calcium_type>(decay_amount_absolute), decay_interval);

    ASSERT_NEAR_EPS(cc1.get_beta(), CalciumCalculator::default_beta);
    ASSERT_NEAR_EPS(cc1.get_tau_C(), CalciumCalculator::default_tau_C);
    ASSERT_EQ(cc1.get_h(), CalciumCalculator::default_h);

    ASSERT_NEAR_EPS(cc2.get_beta(), CalciumCalculator::default_beta);
    ASSERT_NEAR_EPS(cc2.get_tau_C(), CalciumCalculator::default_tau_C);
    ASSERT_EQ(cc2.get_h(), CalciumCalculator::default_h);

    ASSERT_NEAR_EPS(cc3.get_beta(), CalciumCalculator::default_beta);
    ASSERT_NEAR_EPS(cc3.get_tau_C(), CalciumCalculator::default_tau_C);
    ...
    ASSERT_NO_THROW(cc1.set_beta(static_cast<RelearnTypes::calcium_type>(beta1)));
    ASSERT_NO_THROW(cc1.set_tau_C(tau_C1));
    ASSERT_NO_THROW(cc1.set_h(h1));

    ASSERT_NO_THROW(cc2.set_beta(static_cast<RelearnTypes::calcium_type>(beta1)));
    ASSERT_NO_THROW(cc2.set_tau_C(tau_C1));
    ASSERT_NO_THROW(cc2.set_h(h1));

    ASSERT_NO_THROW(cc3.set_beta(static_cast<RelearnTypes::calcium_type>(beta1)));
    ASSERT_NO_THROW(cc3.set_tau_C(tau_C1));
    ASSERT_NO_THROW(cc3.set_h(h1));

    ASSERT_NEAR_EPS(cc1.get_beta(), beta1);
    ASSERT_NEAR_EPS(cc1.get_tau_C(), tau_C1);
    ASSERT_EQ(cc1.get_h(), h1);

    ASSERT_NEAR_EPS(cc2.get_beta(), beta1);
    ASSERT_NEAR_EPS(cc2.get_tau_C(), tau_C1);
    ASSERT_EQ(cc2.get_h(), h1);

    ASSERT_NEAR_EPS(cc3.get_beta(), beta1);
    ASSERT_NEAR_EPS(cc3.get_tau_C(), tau_C1);
    ...
    ASSERT_NO_THROW(cc1.set_beta(static_cast<RelearnTypes::calcium_type>(beta2)));
    ASSERT_NO_THROW(cc1.set_tau_C(tau_C2));
    ASSERT_NO_THROW(cc1.set_h(h2));

    ASSERT_NO_THROW(cc2.set_beta(static_cast<RelearnTypes::calcium_type>(beta2)));
    ASSERT_NO_THROW(cc2.set_tau_C(tau_C2));
    ASSERT_NO_THROW(cc2.set_h(h2));

    ASSERT_NO_THROW(cc3.set_beta(static_cast<RelearnTypes::calcium_type>(beta2)));
    ASSERT_NO_THROW(cc3.set_tau_C(tau_C2));
    ASSERT_NO_THROW(cc3.set_h(h2));

    ASSERT_NEAR_EPS(cc2.get_beta(), beta2);
    ASSERT_NEAR_EPS(cc2.get_tau_C(), tau_C2);
    ...
    ASSERT_NEAR_EPS(cc3.get_beta(), beta2);
    ASSERT_NEAR_EPS(cc3.get_tau_C(), tau_C2);
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorInitialTargetCalcium) {
    ...
    const auto initiator = [number_neurons](mpiPP::MPIRank /*i*/, NeuronID::value_type v) {
        if (v >= number_neurons) {
            return utility::as<calcium_type>(-1.0);
        }

        return static_cast<calcium_type>(v) / utility::as<calcium_type>(7.342);
    };
    ...
    const auto calculator = [number_neurons](mpiPP::MPIRank /*i*/, NeuronID::value_type v) {
        ...
        return static_cast<calcium_type>(v) * utility::as<calcium_type>(5.98);
    };
    ...
    cc1.set_beta(static_cast<RelearnTypes::calcium_type>(beta));
    cc1.set_tau_C(static_cast<CalciumCalculator::calcium_type>(tau_C));
    cc1.set_h(h);
    cc1.set_initial_calcium_calculator(initiator);
    cc1.set_target_calcium_calculator(calculator);

    cc2.set_beta(static_cast<RelearnTypes::calcium_type>(beta));
    cc2.set_tau_C(static_cast<CalciumCalculator::calcium_type>(tau_C));
    cc2.set_h(h);
    cc2.set_initial_calcium_calculator(initiator);
    cc2.set_target_calcium_calculator(calculator);

    cc3.set_beta(static_cast<RelearnTypes::calcium_type>(beta));
    cc3.set_tau_C(static_cast<CalciumCalculator::calcium_type>(tau_C));
    cc3.set_h(h);
    ...
    for (const auto neuron_id : NeuronIDRange::range_id(number_neurons)) {
        const auto initial = static_cast<calcium_type>(neuron_id) / utility::as<calcium_type>(7.342);
        const auto target = static_cast<calcium_type>(neuron_id) * utility::as<calcium_type>(5.98);

        ASSERT_NEAR_EPS(calcium1[neuron_id], initial);
        ASSERT_NEAR_EPS(targets1[neuron_id], target);

        ASSERT_NEAR_EPS(calcium2[neuron_id], initial);
        ASSERT_NEAR_EPS(targets2[neuron_id], target);

        ASSERT_NEAR_EPS(calcium3[neuron_id], initial);
        ASSERT_NEAR_EPS(targets3[neuron_id], target);
    }
}
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorCreate) {
    ...
    const auto initiator_created = [number_neurons, number_created_neurons](mpiPP::MPIRank /*i*/, NeuronID::value_type v) {
        ...
        return static_cast<calcium_type>(v) / utility::as<calcium_type>(1.52);
    };
    ...
    const auto calculator_created = [number_neurons, number_created_neurons](mpiPP::MPIRank /*i*/, NeuronID::value_type v) {
        ...
        return static_cast<calcium_type>(v) * utility::as<calcium_type>(86.2);
    };
    ...
    for (const auto neuron_id : NeuronIDRange::range_id(number_neurons, number_neurons + number_created_neurons)) {
        const auto initial = static_cast<calcium_type>(neuron_id) / utility::as<calcium_type>(1.52);
        const auto target = static_cast<calcium_type>(neuron_id) * utility::as<calcium_type>(86.2);
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateException) {
    ...
    const auto fired = LazySyncedArray<FiredStatus>{ std::vector<FiredStatus>{ FiredStatus::Fired } };

    ASSERT_THROW_NO_PRINT(cc1.update_calcium(step, fired.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc2.update_calcium(step, fired.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc3.update_calcium(step, fired.get_device_ptr_const()), RelearnException);
    ...
    const auto fired_status = LazySyncedArray<FiredStatus>(fired_size);
    ...
    ASSERT_THROW_NO_PRINT(cc1.update_calcium(0, fired_status.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc1.update_calcium(step, fired_status.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc2.update_calcium(0, fired_status.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc2.update_calcium(step, fired_status.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc3.update_calcium(0, fired_status.get_device_ptr_const()), RelearnException);
    ASSERT_THROW_NO_PRINT(cc3.update_calcium(step, fired_status.get_device_ptr_const()), RelearnException);
}
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateNoneDisabled) {
    ...
    const auto neuron_ids = NeuronIDRange::range(number_neurons) | ranges::to_vector;
    extra_info->set_disabled_neurons(neuron_ids);

    cc.set_beta(static_cast<RelearnTypes::calcium_type>(beta));
    cc.set_tau_C(static_cast<CalciumCalculator::calcium_type>(tau_C));
    cc.set_h(h);
    ...
    auto fired_status = LazySyncedArray<FiredStatus>(number_neurons);
    fired_status.resize(number_neurons, FiredStatus::Inactive);
    auto fired_status2 = LazySyncedArray<FiredStatus>(number_neurons);
    fired_status2.resize(number_neurons, FiredStatus::Fired);
    ...
    ASSERT_NO_THROW(cc.update_calcium(0, fired_status.get_device_ptr_const()));
    ...
    ASSERT_NO_THROW(cc.update_calcium(step, fired_status.get_device_ptr_const()));
    ...
    ASSERT_NO_THROW(cc.update_calcium(0, fired_status2.get_device_ptr_const()));
    ...
    ASSERT_NO_THROW(cc.update_calcium(step, fired_status2.get_device_ptr_const()));
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateNoneStep0) {
    ...
    for (const auto neuron_id : NeuronIDRange::range_id(number_neurons)) {
        ...
        if (update_status[neuron_id] == UpdateStatus::Disabled) {
            ASSERT_NEAR_EPS(previous_calcium[neuron_id], now_calcium[neuron_id]);
            continue;
        }

        auto expected_calcium = previous_calcium[neuron_id];
        const auto update_value = fired_status[neuron_id] == FiredStatus::Fired ? beta : calcium_type{ 0 };

        for (auto i = 0U; i < h; i++) {
            expected_calcium = expected_calcium + (calcium_type{ 1 } / static_cast<calcium_type>(h)) * (expected_calcium / -tau_C + update_value);
        }
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateRelativeDisabled) {
    ...
    auto cc = RelativeDecayCalciumCalculator(static_cast<RelearnTypes::calcium_type>(decay_amount_relative), decay_interval);
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateRelativeStep0) {
    ...
    for (const auto neuron_id : NeuronIDRange::range_id(number_neurons)) {
        if (update_status[neuron_id] == UpdateStatus::Disabled) {
            ASSERT_NEAR_EPS(previous_calcium[neuron_id], now_calcium[neuron_id]);
            ASSERT_NEAR_EPS(previous_target[neuron_id], now_target[neuron_id]);
            continue;
        }

        ASSERT_NEAR(static_cast<double>(previous_target[neuron_id]) * decay_amount_relative, now_target[neuron_id], eps);
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateAbsoluteDisabled) {
    ...
    auto cc = AbsoluteDecayCalciumCalculator(static_cast<RelearnTypes::calcium_type>(decay_amount_absolute), decay_interval);
...
TEST_F(CalciumCalculatorTest, testCalciumCalculatorUpdateAbsoluteStep0) {
    ...
    for (const auto neuron_id : NeuronIDRange::range_id(number_neurons)) {
        ...
        ASSERT_NEAR(static_cast<double>(previous_target[neuron_id]) - decay_amount_absolute, now_target[neuron_id], eps);
