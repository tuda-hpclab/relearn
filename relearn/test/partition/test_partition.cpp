...
TEST_F(PartitionTest, testPartitionConstructorArguments) {
    ...
    const auto my_rank = MPIRankFactory::get_random_mpi_rank(mt);
    const auto num_ranks = static_cast<std::size_t>(MPIRankFactory::get_random_number_ranks(mt));
    ...
    if (my_rank.get_rank() >= static_cast<int>(num_ranks)) {
        ASSERT_THROW_NO_PRINT_MSG(Partition part(num_ranks, my_rank), RelearnException, fmt::format("{} {}", num_ranks, my_rank));
        return;
    }
...
TEST_F(PartitionTest, testPartitionConstructor) {
    ...
    for (const auto my_rank : mpiPP::MPIRank::range(num_ranks)) {
        const auto partition = Partition(num_ranks_cast, my_rank);

        ASSERT_EQ(partition.get_number_mpi_ranks(), num_ranks_cast);
...
TEST_F(PartitionTest, testPartitionNumberNeurons) {
    ...
    for (const auto my_rank : ranges::views::indices(num_ranks)) {
        ...
        auto partition = Partition(num_ranks_cast, mpiPP::MPIRank(my_rank));
...
TEST_F(PartitionTest, testPartitionSubdomainIndices) {
    ...
    if (oct_exponent >= std::numeric_limits<std::uint8_t>::max()) {
        return;
    }

    const auto oct_exponent_cast = static_cast<std::uint8_t>(oct_exponent);
    const auto sfc = Morton(oct_exponent_cast);

    auto found_indices = std::vector<bool>(num_subdomains, false);

    for (const auto my_rank : ranges::views::indices(num_ranks)) {
        const auto partition = Partition(num_ranks_cast, mpiPP::MPIRank(my_rank));
...
TEST_F(PartitionTest, testPartitionSubdomainBoundaries) {
    ...
    for (const auto my_rank : ranges::views::indices(num_ranks)) {
        ...
        ASSERT_THROW_NO_PRINT(partition.set_simulation_box_size({ { simulation_box_minimum.get_x() + Constants::uninitialized, simulation_box_minimum.get_y(), simulation_box_minimum.get_z() }, simulation_box_maximum }), RelearnException);
        ASSERT_THROW_NO_PRINT(partition.set_simulation_box_size({ { simulation_box_minimum.get_x(), simulation_box_minimum.get_y() + Constants::uninitialized, simulation_box_minimum.get_z() }, simulation_box_maximum }), RelearnException);
        ASSERT_THROW_NO_PRINT(partition.set_simulation_box_size({ { simulation_box_minimum.get_x(), simulation_box_minimum.get_y(), simulation_box_minimum.get_z() + Constants::uninitialized }, simulation_box_maximum }), RelearnException);

        ASSERT_THROW_NO_PRINT(partition.set_simulation_box_size({ simulation_box_minimum, { simulation_box_maximum.get_x() + Constants::uninitialized, simulation_box_maximum.get_y(), simulation_box_maximum.get_z() } }), RelearnException);
        ASSERT_THROW_NO_PRINT(partition.set_simulation_box_size({ simulation_box_minimum, { simulation_box_maximum.get_x(), simulation_box_maximum.get_y() + Constants::uninitialized, simulation_box_maximum.get_z() } }), RelearnException);
        ASSERT_THROW_NO_PRINT(partition.set_simulation_box_size({ simulation_box_minimum, { simulation_box_maximum.get_x(), simulation_box_maximum.get_y(), simulation_box_maximum.get_z() + Constants::uninitialized } }), RelearnException);
    ...
    for (const auto my_rank : ranges::views::indices(num_ranks)) {
        ...
        for (auto subdomain_index_1 = std::size_t{ 0 }; subdomain_index_1 < num_subdomains; subdomain_index_1++) {
            ...
            const auto subdomain_expected_min = Vec3d{
                subdomain_box_dimensions.get_x() * static_cast<double>(subdomain_index_3.get_x()),
                subdomain_box_dimensions.get_y() * static_cast<double>(subdomain_index_3.get_y()),
                subdomain_box_dimensions.get_z() * static_cast<double>(subdomain_index_3.get_z())
...
TEST_F(PartitionTest, testMemoryFootprint) {
    ...
    const auto part = Partition(num_ranks_cast, mpiPP::MPIRank::root_rank());
...
TEST_F(PartitionTest, testSetBoundaryCorrector) {
    ...
    auto part = Partition(num_ranks_cast, mpiPP::MPIRank::root_rank());

    ASSERT_THROW_NO_PRINT(part.set_boundary_correction_function({}), RelearnException);
    auto corrector = [](RelearnTypes::box_size_type /*val*/) -> RelearnTypes::box_size_type {
        return RelearnTypes::box_size_type{ 1.0 };
    };
...
TEST_F(PartitionTest, testCorrectedSubdomainBoundaries) {
    ...
    for (auto my_rank = 0; my_rank < num_ranks; my_rank++) {
        ...
        for (auto subdomain_index_1 = std::size_t{ 0 }; subdomain_index_1 < num_subdomains; subdomain_index_1++) {
            ...
            ASSERT_EQ(min_1, RelearnTypes::box_size_type(1.0));
            ASSERT_EQ(max_1, RelearnTypes::box_size_type(1.0));
        }

        partition.calculate_and_set_subdomain_boundaries();

        const auto local_subdomain_boundaries = std::vector<std::pair<RelearnTypes::box_size_type, RelearnTypes::box_size_type>>{};
        ...
        for (auto my_subdomain = 0U; my_subdomain < my_subdomains; my_subdomain++) {
            ...
            ASSERT_EQ(min1, RelearnTypes::box_size_type(1.0));
            ASSERT_EQ(max1, RelearnTypes::box_size_type(1.0));
        }
    }
}
