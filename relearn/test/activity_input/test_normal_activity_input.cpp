...
TEST_F(NormalActivityInputTest, testConstructorNoThrow) {
    ...
    const auto mean_input = RandomFactory::get_random_double<double>(NormalActivityInput::min_mean_activity, NormalActivityInput::max_mean_activity, this->mt);
    const auto stddev_input = RandomFactory::get_random_double<double>(NormalActivityInput::min_stddev_activity, NormalActivityInput::max_stddev_activity, this->mt);

    ASSERT_NO_THROW(std::ignore = NormalActivityInput(0.0, 1.0));
    ASSERT_NO_THROW(std::ignore = NormalActivityInput(1.0, 1.0));
    ASSERT_NO_THROW(std::ignore = NormalActivityInput(-1.0, 1.0));
    ASSERT_NO_THROW(std::ignore = NormalActivityInput(0.0, stddev_input));
    ASSERT_NO_THROW(std::ignore = NormalActivityInput(mean_input, 1.0));
    ASSERT_NO_THROW(std::ignore = NormalActivityInput(-mean_input, 1.0));
    ASSERT_NO_THROW(std::ignore = NormalActivityInput(mean_input, stddev_input));
}
...
TEST_F(NormalActivityInputTest, testConstructorThrow) {
    ...
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(0.0, -1.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(1.0, -1.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(-1.0, -1.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(0.0, -stddev_input), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(mean_input, 0.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(-mean_input, 0.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(mean_input, -1.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(-mean_input, -1.0), RelearnException);
    ASSERT_THROW_NO_PRINT(std::ignore = NormalActivityInput(mean_input, -stddev_input), RelearnException);
}
...
TEST_F(NormalActivityInputTest, testInitAndCreate) {
    ...
    auto normal_activity_input = NormalActivityInput(mean_input, stddev_input);
