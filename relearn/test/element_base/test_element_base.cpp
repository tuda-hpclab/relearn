...
TEST_F(ElementBaseTest, testSetCalculatorsInit) {
    ...
    const auto grown_elements_calculator = [](const RelearnTypes::number_neurons_type i) -> double {
        return static_cast<double>(i);
    };

    const auto delta_since_last_update_calculator = [](const RelearnTypes::number_neurons_type i) -> double {
    ...
    const auto vacant_retract_ratio_calculator = [](const RelearnTypes::number_neurons_type i) -> double {
    ...
    const auto minimum_calcium_calculator = [](const RelearnTypes::number_neurons_type i) -> RelearnTypes::calcium_type {
        return static_cast<RelearnTypes::calcium_type>(i) * 0.1;
    };
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        ...
        ASSERT_EQ(retract_ratio[i], vacant_retract_ratio_calculator(i));
        ASSERT_EQ(minimum_calcum[i], minimum_calcium_calculator(i));

        if (connected_elements[i] >= grown_elements[i]) {
            ASSERT_EQ(vacant_elements[i], 0U);
        } else {
            ASSERT_EQ(vacant_elements[i], static_cast<unsigned int>(grown_elements[i] - connected_elements[i]));
        }
    }
}
...
TEST_F(ElementBaseTest, testAddToDelta) {
    ...
    auto deltas_to_add = std::vector<double>{ 0.0, -4.2, 6.01, 1.0 };
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        ASSERT_EQ(grown_elements[i], 0.0);
        ASSERT_EQ(deltas[i], 2.0 * deltas_to_add[i]);
        ASSERT_EQ(vacant_elements[i], 0U);
        ASSERT_EQ(connected_elements[i], 0U);
        ASSERT_EQ(retract_ratio[i], 0.0);
        ASSERT_EQ(minimum_calcum[i], 0.0);
...
TEST_F(ElementBaseTest, testAddToDeltaException) {
    ...
    auto deltas_to_add = std::vector<double>{ 0.0, -4.2, 6.01, 1.0 };
    auto deltas_to_add_2 = std::vector<double>{ 0.0, -4.2, 6.01, 1.0, 9.5 };
    ...
    for (auto i = number_neurons_init; i < number_neurons_init + number_neurons_init; i++) {
        ASSERT_THROW_NO_PRINT(element_base.add_to_delta(0.3, i), RelearnException);
    }
...
TEST_F(ElementBaseTest, testAddConnectedElements) {
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr auto expected_grown = std::array{ 0.0, 4.0, 6.0, 1.0 };
...
TEST_F(ElementBaseTest, testRemoveConnectedElements) {
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr auto expected_grown = std::array{ 10.0, 10.0, 10.0, 10.0 };
...
TEST_F(ElementBaseTest, testConnectElements) {
    ...
    auto deltas_to_add = std::vector<double>{ 21.5, 12.5, 16.01, 72.0 };
    element_base.add_to_delta(deltas_to_add);
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr auto expected_grown = std::array{ 21.5, 12.5, 16.01, 72.0 };
...
TEST_F(ElementBaseTest, testCommitUpdates1) {
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr static auto expected_grown = std::array{ 0.0, 0.0, 6.01, 1.0 };
...
TEST_F(ElementBaseTest, testCommitUpdates2) {
    ...
    auto deltas_to_add_1 = std::vector<double>{ 0.0, -4.2, 6.01, 1.0 };
    ...
    auto deltas_to_add_2 = std::vector<double>{ 0.4, 1.3, -4.002, -0.2 };
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr static auto expected_grown = std::array{ 0.4, 1.3, 2.008, 0.8 };
...
TEST_F(ElementBaseTest, testCommitUpdates3) {
    ...
    auto connected_to_add = std::vector{ 10U, 10U, 10U, 9U };
    auto connected_to_remove = std::vector{ 4U, 8U, 2U, 0U };
    auto delta_to_add = std::vector{ 0.2, 0.3, 0.9, 0.7 };
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr static auto expected_grown = std::array{ 10.2, 10.3, 10.9, 9.7 };
    ...
    auto deltas_to_add_2 = std::vector{ -3.7, 2.1, -4.5, -12.1 };
    element_base.add_to_delta(deltas_to_add_2);
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr static auto expected_grown = std::array{ 6.5, 12.4, 6.4, 0.0 };
...
TEST_F(ElementBaseTest, testCommitUpdates4) {
    ...
    auto deltas_to_add_2 = std::vector<double>{ 2.3, 1.8, 0.4, 0.2 };
    element_base.add_to_delta(deltas_to_add_2);
    std::ignore = element_base.commit_updates();
    ...
    auto deltas_to_add_3 = std::vector<double>{ 0.2, -0.4, -2.43, 0.4 };
    ...
    auto deltas_to_add_4 = std::vector<double>{ 0.0, 0.0, 0.0, 0.0 };
    ...
    auto deltas_to_add_5 = std::vector<double>{ 0.45, 0.22, 1.9, -0.5 };
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr static auto expected_grown = std::array{ 2.95, 1.62, 5.88, 1.1 };
...
TEST_F(ElementBaseTest, testDisableNeurons) {
    ...
    auto deltas_to_add_6 = std::vector<double>{ 0.45, 0.22, 1.9, -0.5 };
    ...
    for (auto i = RelearnTypes::number_neurons_type{ 0 }; i < number_neurons_init; i++) {
        constexpr static auto expected_grown = std::array{ 2.95, 0.0, 5.88, 0.0 };
        constexpr static auto expected_vacant = std::array{ 2U, 0U, 5U, 0U };
        constexpr static auto expected_delta = std::array{ 0.45, 0.0, 1.9, 0.0 };

        ASSERT_NEAR(grown_elements[i], expected_grown[i], eps);
        ASSERT_EQ(deltas[i], expected_delta[i]);
        ASSERT_EQ(vacant_elements[i], expected_vacant[i]);
        ASSERT_EQ(connected_elements[i], 0U);
        ASSERT_EQ(retract_ratio[i], 0.0);
        ASSERT_EQ(minimum_calcum[i], 0.0);
...
TEST_F(ElementBaseTest, testDisableNeuronsException) {
    ...
    const auto deltas_to_add_1 = std::vector<double>{ 0.0, -4.2, 6.01, 1.0 };
    element_base.add_to_delta(deltas_to_add_1);
    std::ignore = element_base.commit_updates();
