...
public:
    ...
    [[nodiscard]] const std::vector<RequestType>& get_requests(const MPIRank mpi_rank) const {
        utility::Exception::check(mpi_rank.is_initialized(), "CommunicationMap::contains: get_requests is not initialized.");
    ...
    [[nodiscard]] std::optional<std::reference_wrapper<const std::vector<RequestType>>> get_optional_requests(const MPIRank mpi_rank) const {
        utility::Exception::check(mpi_rank.is_initialized(), "CommunicationMap::contains: get_optional_requests is not initialized.");
    ...
    void resize(const std::span<const requests_size_type> sizes) {
        utility::Exception::check(sizes.size() <= static_cast<size_type>(number_ranks),
                                  "CommunicationMap::resize: number of sizes {} is larger than the number of ranks {}", sizes.size(), number_ranks);
     ...
     * @exception Throws a Exception if any included MPI rank is too large
     */
    ...
    [[nodiscard]] std::vector<requests_size_type> get_request_sizes_vector() const noexcept {
        ...
        for (const auto& [rank, requests_for_rank] : requests) {
            number_requests[utility::save_cast<std::size_t>(rank.get_rank())] = requests_for_rank.size();
        }
