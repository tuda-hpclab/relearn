...
public:
     ...
     * @exception Throws a Exception if number_ranks is smaller than 1
     */
    explicit CommunicationVector(const int num_ranks, [[maybe_unused]] const size_type size_hint = 1)
        : number_ranks(num_ranks)
        , requests(utility::save_cast<std::size_t>(num_ranks), std::vector<RequestType>{}) {
     ...
     * @exception Throws a Exception if mpi_rank is negative or too large with respect to the number of ranks
    ...
    [[nodiscard]] size_type size() const noexcept {
        ...
        return utility::save_cast<size_type>(counted);
    }
    ...
    [[nodiscard]] const std::vector<RequestType>& get_requests(const MPIRank mpi_rank) const {
        utility::Exception::check(mpi_rank.is_initialized(), "CommunicationVector::contains: get_requests is not initialized.");
    ...
    [[nodiscard]] std::optional<std::reference_wrapper<const std::vector<RequestType>>> get_optional_requests(const MPIRank mpi_rank) const {
        utility::Exception::check(mpi_rank.is_initialized(), "CommunicationVector::contains: get_optional_requests is not initialized.");
     ...
     * @exception Throws a Exception if mpi_rank is uninitialized or too large with respect to the number of ranks
     */
     ...
     * @exception Throws a Exception if sizes.size() > number_ranks
     */
    void resize(const std::span<const requests_size_type> sizes) {
        utility::Exception::check(sizes.size() <= static_cast<size_type>(number_ranks),
                                  "CommunicationVector::resize: number of sizes {} is larger than the number of ranks {}", sizes.size(), number_ranks);

        for (const auto mpi_rank : MPIRank::range(number_ranks)) {
            const auto size_for_rank = sizes[utility::save_cast<std::size_t>(mpi_rank.get_rank())];
            requests[mpi_rank.get_rank_cast()].resize(size_for_rank);
        }
    }
    ...
    void resize(sizes_type sizes) {
        requests.clear();
        requests.resize(utility::save_cast<std::size_t>(number_ranks));
     ...
     * @exception Throws a Exception if mpi_rank is negative or too large with respect to the number of ranks
     * @exception Throws a Exception if mpi_rank is negative, the rank does not have saved requests, or the value is too large with respect to the number of ranks
     */
    ...
    [[nodiscard]] sizes_type get_request_sizes() const {
        ...
        number_requests.reserve(utility::save_cast<std::size_t>(number_ranks));
