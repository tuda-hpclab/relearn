 ...
 *      Holds everything that is common to the CPU and GPU element bases; ElementBaseCPU and
 *      ElementBaseGPU add the parts that differ (committing updates and reporting the device-side
 *      memory footprint).
 *      Templated on the per-neuron array storage (Storage<T>): ElementBaseCPU instantiates it
 *      with std::vector, ElementBaseGPU with LazySyncedArray.
 */
template <template <typename...> class Storage>
class ElementBaseBase {
public:
    ...
    ElementBaseBase() = default;

    ElementBaseBase(const ElementBaseBase& other) = delete;
    ElementBaseBase(ElementBaseBase&& other) = default;

    ElementBaseBase& operator=(const ElementBaseBase& other) = delete;
    ElementBaseBase& operator=(ElementBaseBase&& other) = default;
    ...
    void set_grown_elements_calculator(std::function<grown_type(number_neurons_type)> calculator) {
        ...
        RelearnException::check(function_full, "ElementBaseBase::set_grown_elements_calculator: calculator is empty");
    ...
    void set_delta_since_last_update_calculator(std::function<grown_type(number_neurons_type)> calculator) {
        const auto function_full = calculator.operator bool();
        RelearnException::check(function_full, "ElementBaseBase::set_delta_since_last_update_calculator: calculator is empty");
    ...
    void set_connected_elements_calculator(std::function<counter_type(number_neurons_type)> calculator) {
        const auto function_full = calculator.operator bool();
        RelearnException::check(function_full, "ElementBaseBase::set_connected_elements_calculator: calculator is empty");
    ...
    void set_vacant_retract_ratio_calculator(std::function<grown_type(number_neurons_type)> calculator) {
        const auto function_full = calculator.operator bool();
        RelearnException::check(function_full, "ElementBaseBase::set_vacant_retract_ratio_calculator: calculator is empty");
    ...
    void set_minimum_calcium_calculator(std::function<calcium_type(number_neurons_type)> calculator) {
        const auto function_full = calculator.operator bool();
        RelearnException::check(function_full, "ElementBaseBase::set_minimum_calcium_calculator: calculator is empty");
    ...
    void set_extra_infos(std::shared_ptr<NeuronsExtraInfo> new_extra_info) {
        ...
        RelearnException::check(is_filled, "ElementBaseBase::set_extra_infos: new_extra_info is empty");
    ...
    void create_neurons(const RelearnTypes::number_neurons_type creation_count) {
        RelearnException::check(size > 0, "ElementBaseBase::create_neurons: Not initialized");
        RelearnException::check(creation_count > 0, "ElementBaseBase::create_neurons: creation_count must be > 0");
     ...
     */
    void disable_neurons(const std::span<const RelearnTypes::number_neurons_type> disabled_neuron_ids) {
        for (const auto neuron_id : disabled_neuron_ids) {
            RelearnException::check(neuron_id < size, "ElementBaseBase::disable_neurons: Cannot disable a neuron with a too large id");
    ...
    void add_to_delta(const grown_type delta, const number_neurons_type neuron_id) {
        RelearnException::check(neuron_id < size, "ElementBaseBase::add_to_delta: neuron_id is too large: {}", neuron_id);
    ...
    void add_to_delta(const std::span<const grown_type> deltas) {
        RelearnException::check(deltas.size() == size, "ElementBaseBase::add_to_delta: deltas.size() != size");
    ...
    void add_connected_elements(const counter_type newly_added, const number_neurons_type neuron_id) {
        RelearnException::check(neuron_id < size, "ElementBaseBase::add_connected_elements: neuron_id is too large: {}", neuron_id);
        connected_elements[neuron_id] += newly_added;
        grown_elements[neuron_id] += static_cast<grown_type>(newly_added);
    }
    ...
    void add_connected_elements(const std::span<const counter_type> changes) {
        RelearnException::check(changes.size() == size, "ElementBaseBase::add_connected_elements: changes.size() != size");

        for (auto neuron_id = RelearnTypes::number_neurons_type{ 0 }; neuron_id < size; neuron_id++) {
            const auto change = changes[neuron_id];
            connected_elements[neuron_id] += change;
            grown_elements[neuron_id] += static_cast<grown_type>(change);
        }
    }
    ...
    void remove_connected_elements(const counter_type newly_free, const number_neurons_type neuron_id) {
        RelearnException::check(neuron_id < size, "ElementBaseBase::remove_connected_elements: neuron_id is too large: {}", neuron_id);
        RelearnException::check(connected_elements[neuron_id] >= newly_free,
                                "ElementBaseBase::remove_connected_elements: Cannot delete more connections than present for neuron {}: {} vs {}", neuron_id, newly_free, connected_elements[neuron_id]);
        RelearnException::check(static_cast<counter_type>(grown_elements[neuron_id]) >= newly_free,
                                "ElementBaseBase::remove_connected_elements: Cannot delete more connections than present for neuron {}: {} vs {}", neuron_id, newly_free, connected_elements[neuron_id]);
    ...
    void remove_connected_elements(const std::span<const counter_type> changes) {
        ...
        for (auto neuron_id = RelearnTypes::number_neurons_type{ 0 }; neuron_id < size; neuron_id++) {
            ...
            RelearnException::check(connected_elements[neuron_id] >= change,
                                    "ElementBaseBase::remove_connected_elements: Cannot delete more connections than present for neuron {}: {} vs {}", neuron_id, change, connected_elements[neuron_id]);
    ...
    void connect_elements(const counter_type newly_connected, const number_neurons_type neuron_id) {
        RelearnException::check(neuron_id < size, "ElementBaseBase::connect_elements: neuron_id is too large: {}", neuron_id);

        const auto vacant = vacant_elements[neuron_id];
        RelearnException::check(vacant >= newly_connected, "ElementBaseBase::connect_elements: Not enough vacant elements for neuron {}: {} vs {}", neuron_id, newly_connected, vacant);
    ...
    void connect_elements(const std::span<const counter_type> changes) {
        RelearnException::check(changes.size() == size, "ElementBaseBase::connect_elements: changes.size() != size");

        for (auto neuron_id = RelearnTypes::number_neurons_type{ 0 }; neuron_id < size; neuron_id++) {
            const auto change = changes[neuron_id];
            const auto vacant = vacant_elements[neuron_id];

            RelearnException::check(vacant >= change, "ElementBaseBase::connect_elements: Not enough vacant elements for neuron {}: {} vs {}", neuron_id, change, vacant);
    ...
    void disconnect_elements(const counter_type newly_disconnected, const number_neurons_type neuron_id) {
        RelearnException::check(neuron_id < size, "ElementBaseBase::disconnect_elements: neuron_id is too large: {}", neuron_id);

        const auto connected = connected_elements[neuron_id];
        RelearnException::check(connected >= newly_disconnected, "ElementBaseBase::disconnect_elements: Not enough connected elements for neuron {}: {} vs {}", neuron_id, newly_disconnected, connected);
    ...
    void disconnect_elements(const std::span<const counter_type> changes) {
        RelearnException::check(changes.size() == size, "ElementBaseBase::disconnect_elements: changes.size() != size");

        for (auto neuron_id = RelearnTypes::number_neurons_type{ 0 }; neuron_id < size; neuron_id++) {
            const auto change = changes[neuron_id];
            const auto connected = connected_elements[neuron_id];

            RelearnException::check(connected >= change, "ElementBaseBase::disconnect_elements: Not enough connected elements for neuron {}: {} vs {}", neuron_id, change, connected);
...
protected:
    void init_values(const RelearnTypes::number_neurons_type begin, const RelearnTypes::number_neurons_type end) {
        for (auto i = begin; i < end; i++) {
            ...
            RelearnException::check(forced_grown_elements >= grown_type{ 0 }, "ElementBaseBase::init_values: grown_elements_calculator returned {} for id {}", forced_grown_elements, i);
            RelearnException::check(forced_vacant_retract_ratio >= grown_type{ 0 }, "ElementBaseBase::init_values: vacant_retract_ratio_calculator returned {} for id {}", forced_vacant_retract_ratio, i);
            RelearnException::check(forced_vacant_retract_ratio <= grown_type{ 1 }, "ElementBaseBase::init_values: vacant_retract_ratio_calculator returned {} for id {}", forced_vacant_retract_ratio, i);
            RelearnException::check(forced_minimum_calcium >= calcium_type{ 0 }, "ElementBaseBase::init_values: minimum_calcium_calculator returned {} for id {}", forced_minimum_calcium, i);

            const auto cast_grown_elements = static_cast<counter_type>(forced_grown_elements);
            ...
            if (forced_connected_elements >= cast_grown_elements) { // NOLINT(bugprone-branch-clone) - underflow guard for unsigned subtraction, branches compute different values
                vacant_elements[i] = 0;
            } else {
    ...
    [[nodiscard]] counter_type update(const number_neurons_type neuron_id) {
        ...
        const auto current_connected_count = static_cast<grown_type>(current_connected_count_integral);
        const auto current_vacant = current_count - current_connected_count;

        RelearnException::check(current_count >= grown_type{ 0 }, "ElementBaseBase::update: {}", current_count);
        RelearnException::check(current_connected_count >= grown_type{ 0 }, "ElementBaseBase::update: {}", current_connected_count);
        RelearnException::check(current_vacant >= grown_type{ 0 }, "ElementBaseBase::update: {}", current_count - current_connected_count);

        if (current_delta >= grown_type{ 0 }) {
            total_additions += current_delta;
        } else {
            total_deletions += std::min(-current_delta, current_count);
        }

        if (const auto new_vacant = current_vacant + current_delta; new_vacant >= grown_type{ 0 }) {
            ...
            RelearnException::check(new_count >= current_connected_count, "ElementBaseBase::update: new count is smaller than connected count");

            grown_elements[neuron_id] = new_count;
            delta_since_last_update[neuron_id] = 0.0;
            vacant_elements[neuron_id] = static_cast<counter_type>(new_vacant_after_retract);
            // connected_elements does not need to change

            return 0U;
        }

        if (current_count + current_delta <= grown_type{ 0 }) {
            ...
            grown_elements[neuron_id] = grown_type{ 0 };
            delta_since_last_update[neuron_id] = grown_type{ 0 };
        ...
        RelearnException::check(num_vacant >= grown_type{ 0 }, "ElementBaseBase::update: num_vacant is negative");
        RelearnException::check(num_vacant < grown_type{ 1 }, "ElementBaseBase::update: num_vacant is larger than 1.0");

        grown_elements[neuron_id] = retracted_new_count;
        delta_since_last_update[neuron_id] = grown_type{ 0 };
        vacant_elements[neuron_id] = 0U;
        connected_elements[neuron_id] = static_cast<counter_type>(new_connected_count);

        const auto deleted_counts = current_connected_count - new_connected_count;

        RelearnException::check(deleted_counts >= grown_type{ 0 }, "ElementBaseBase::update: deleted was negative");
        const auto num_delete_connected = static_cast<counter_type>(deleted_counts);

        return num_delete_connected;
    }

    number_neurons_type size{};
    grown_type total_additions{ 0.0 };
    grown_type total_deletions{ 0.0 };

    std::shared_ptr<NeuronsExtraInfo> extra_infos;

    Storage<grown_type> grown_elements{};
    Storage<grown_type> delta_since_last_update{};
    Storage<counter_type> vacant_elements{};
    Storage<counter_type> connected_elements{};

    Storage<grown_type> vacant_retract_ratio{};
    Storage<calcium_type> minimum_calcium{};

    std::function<grown_type(number_neurons_type)> grown_elements_calculator{ details::get_zero<grown_type> };
    std::function<grown_type(number_neurons_type)> delta_since_last_update_calculator{ details::get_zero<grown_type> };
    std::function<counter_type(number_neurons_type)> connected_elements_calculator{ details::get_zero<counter_type> };

    std::function<grown_type(number_neurons_type)> vacant_retract_ratio_calculator{ details::get_zero<grown_type> };
    std::function<calcium_type(number_neurons_type)> minimum_calcium_calculator{ details::get_zero<calcium_type> };
};
