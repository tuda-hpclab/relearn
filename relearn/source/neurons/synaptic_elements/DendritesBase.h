...
/**
 * @brief Holds everything that is common to the CPU and GPU dendrites; DendritesCPU and
 *      DendritesGPU add the parts that differ (get_cuda_handle(_const) only exists on GPU, and
...
public:
    ...
    DendritesBase() = default;

    DendritesBase(const DendritesBase& other) = delete;
    DendritesBase(DendritesBase&& other) = default;

    DendritesBase& operator=(const DendritesBase& other) = delete;
    DendritesBase& operator=(DendritesBase&& other) = default;

    virtual ~DendritesBase() = default;
    ...
    void set_extra_infos(std::shared_ptr<NeuronsExtraInfo> new_extra_info) {
        ...
        excitatory_dendrites.set_extra_infos(extra_infos);
        inhibitory_dendrites.set_extra_infos(extra_infos);
    }
     ...
     */
    void set_grown_elements_calculator(std::function<grown_type(number_neurons_type)> calculator, const SignalType signal_type) {
        ...
        if (signal_type == SignalType::Excitatory) { // NOLINT(bugprone-branch-clone) - dispatches to a different member object (excitatory_dendrites vs inhibitory_dendrites), not identical branches
     ...
     * @exception Throws a RelearnException if calculator is empty
     */
    void set_delta_since_last_update_calculator(std::function<grown_type(number_neurons_type)> calculator, const SignalType signal_type) {
        const auto function_full = calculator.operator bool();
        RelearnException::check(function_full, "Dendrites::set_delta_since_last_update_calculator: calculator is empty");

        if (signal_type == SignalType::Excitatory) { // NOLINT(bugprone-branch-clone) - dispatches to a different member object, not identical branches
     ...
     * @exception Throws a RelearnException if calculator is empty
     */
    void set_connected_elements_calculator(std::function<counter_type(number_neurons_type)> calculator, const SignalType signal_type) {
        const auto function_full = calculator.operator bool();
     ...
     * @exception Throws a RelearnException if calculator is empty
     */
    void set_vacant_retract_ratio_calculator(std::function<grown_type(number_neurons_type)> calculator, const SignalType signal_type) {
        const auto function_full = calculator.operator bool();
     ...
     * @exception Throws a RelearnException if calculator is empty
     */
    void set_minimum_calcium_calculator(std::function<calcium_type(number_neurons_type)> calculator, const SignalType signal_type) {
        const auto function_full = calculator.operator bool();
     ...
     */
    void add_to_delta(const grown_type delta, const number_neurons_type neuron_id, const SignalType signal_type) {
     ...
     */
    void add_to_delta(const std::span<const grown_type> deltas, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     * @exception Throws a RelearnException if neuron_id is too large
     */
    void add_connected_elements(const counter_type newly_added, const number_neurons_type neuron_id, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void add_connected_elements(const std::span<const counter_type> changes, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void remove_connected_elements(const counter_type newly_free, const number_neurons_type neuron_id, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void remove_connected_elements(const std::span<const counter_type> changes, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void connect_elements(const counter_type newly_connected, const number_neurons_type neuron_id, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void connect_elements(const std::span<const counter_type> changes, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void disconnect_elements(const counter_type newly_disconnected, const number_neurons_type neuron_id, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    void disconnect_elements(const std::span<const counter_type> changes, const SignalType signal_type) {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] grown_type get_total_additions(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] grown_type get_total_deletions(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] std::span<const grown_type> get_grown_elements(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] std::span<const grown_type> get_deltas(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] std::span<const counter_type> get_vacant_elements(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] std::span<const counter_type> get_connected_elements(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] std::span<const grown_type> get_vacant_retract_ratio(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
     ...
     */
    [[nodiscard]] std::span<const calcium_type> get_minimum_calcium(const SignalType signal_type) const noexcept {
        if (signal_type == SignalType::Excitatory) {
    ...
    void record_memory_footprint(const std::unique_ptr<utility::MemoryFootprint>& footprint) {
        ...
        footprint->emplace("Dendrites", my_size);
    }
