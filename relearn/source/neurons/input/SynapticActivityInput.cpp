...
void SynapticActivityInput::update_input_range(step_type /*step*/, NeuronID first, NeuronID last) {
    Timers::start(TimerRegion::CALC_ACTIVITY_INPUT_SYNAPSES);

    const auto& extra_infos = get_extra_infos();

    RelearnException::check(extra_infos != nullptr, "SynapticActivityInput::update_input_range: extra_infos is empty");
    RelearnException::check(fired_status_comm != nullptr, "SynapticActivityInput::update_input_range: fired_status_comm is empty");
    RelearnException::check(fired_status_comm->get_network_graph() != nullptr, "SynapticActivityInput::update_input_range: network_graph of fired_status_comm is empty");
    ...
    RelearnException::check(num_local_neurons > 0,
                            "SynapticActivityInput::update_input_range: There were no local neurons.");
    RelearnException::check(disable_flags.size() == num_local_neurons,
                            "SynapticActivityInput::update_input_range: Size of disable_flags did not match number of local neurons: {} vs {}", disable_flags.size(), num_local_neurons);
    RelearnException::check(fired_status.size() == num_local_neurons,
                            "SynapticActivityInput::update_input_range: Size of fired_status did not match number of local neurons: {} vs {}", fired_status.size(), num_local_neurons);
...
void SynapticActivityInput::register_neuron_monitor(NeuronMonitor& monitor) {
