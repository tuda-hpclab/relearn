...
#include "SynapticIndividuallyWeightedActivityInput.h"

#include "neurons/NetworkGraph.h"

#include <utility>

void SynapticIndividuallyWeightedActivityInput::update_local_input(const std::span<const FiredStatus> fired, std::span<double> input, NeuronID first, NeuronID last) {
    ...
    for (const auto neuron_id : NeuronID::range(first, last)) {
        ...
        for (const auto& [target_neuron_id, synapse_strength] : local_out_edges_static) {
            input[target_neuron_id.get_neuron_id()] += synapse_strength; // no synaptic plasticity used here (yet)
        }
        ...
        for (const auto& [target_neuron_id, synapse_strength] : local_out_edges_plastic) {
            ...
            auto sum_of_weights = SynapticIndividuallyWeightedActivityInput::weight_type{ 0 };
            for (const auto& weight : synapse_weight_vector) {
                sum_of_weights += weight;
            }
            input[target_neuron_id.get_neuron_id()] += sum_of_weights;
        }
    }
}

void SynapticIndividuallyWeightedActivityInput::update_distant_input(const std::span<const FiredStatus> /*fired*/, std::span<double> input, NeuronID first, NeuronID last) {
    const auto& network_graph = get_network_graph();

    for (const auto neuron_id : NeuronID::range(first, last)) {
        auto local_plastic_input = SynapticIndividuallyWeightedActivityInput::weight_type{ 0 };
...
[[nodiscard]] const SynapticIndividuallyWeightedActivityInput::weight_vector_type& SynapticIndividuallyWeightedActivityInput::get_weights(const SynapticIndividuallyWeightedActivityInput::weight_map_type& weight_map_to_use, const NeuronID target_neuron, const RankNeuronId source_neuron) const {
...
[[nodiscard]] const SynapticIndividuallyWeightedActivityInput::weight_vector_type& SynapticIndividuallyWeightedActivityInput::get_weights(const SynapticIndividuallyWeightedActivityInput::weight_map_type& weight_map_to_use, const NeuronID::value_type target_neuron, const RankNeuronId source_neuron) const {
