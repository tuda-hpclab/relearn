...
#include "SynapticScalingActivityInput.h"
...
void SynapticScalingActivityInput::ensure_scales() {
    ...
    for (const auto neuron_id : NeuronID::range(scales_size)) {
        ...
        const auto s1 = std::abs(static_cast<double>(plastic_excitatory_in_edges));
        const auto s2 = std::abs(static_cast<double>(plastic_inhibitory_in_edges));

        const auto sum = s1 + s2;

        scales[id] = t_scale / (sum == 0.0 ? 1.0 : sum);
    }

    needs_scale_update = false;
}

void SynapticScalingActivityInput::update_local_input(std::span<const FiredStatus> fired, std::span<double> input, const NeuronID first, const NeuronID last) {
    ...
    for (const auto neuron_id : NeuronID::range(first, last)) {
        ...
        for (const auto& [src_neuron_id, synapse_strength] : local_in_edges_plastic) {
            ...
            const auto scaled = static_cast<double>(synapse_strength) * scale;
            input[id] += scaled;
        }
    }
}

void SynapticScalingActivityInput::update_distant_input(std::span<const FiredStatus> /*fired*/, std::span<double> input, const NeuronID first, const NeuronID last) {
    ...
    for (const auto neuron_id : NeuronID::range(first, last)) {
        ...
        const auto local_input = (scale * static_cast<double>(local_plastic_input)) + local_static_input;
