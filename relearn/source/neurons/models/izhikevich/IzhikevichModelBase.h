 ...
 *      v(t) = c
 *      u(t) += d
 *      Holds everything that is common to the CPU and GPU Izhikevich models; IzhikevichModelCPU
 *      and IzhikevichModelGPU add the parts that differ (update_activity(_benchmark)).
 */
class IzhikevichModelBase : public NeuronModel {
public:
    /**
     * @brief Constructs a new instance of type IzhikevichModelBase with 0 neurons and the passed values for all parameters.
     ...
     */
    [[nodiscard]] activity_type get_u(const NeuronID neuron_id) const {
     ...
     */
    [[nodiscard]] const models::izhikevich::Parameters<RelearnTypes::activity_type>& get_model_parameters() const noexcept {
        return parameters;
    }
...
protected:
    ...
    LazySyncedArray<activity_type> u; // membrane recovery

    models::izhikevich::Parameters<RelearnTypes::activity_type> parameters;
};
