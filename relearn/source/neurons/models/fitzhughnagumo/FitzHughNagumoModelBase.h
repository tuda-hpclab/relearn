 ...
 *      Holds everything that is common to the CPU and GPU FitzHughNagumo models; FitzHughNagumoModelCPU
 *      and FitzHughNagumoModelGPU add the parts that differ (update_activity(_benchmark)).
 */
class FitzHughNagumoModelBase : public NeuronModel {
public:
    /**
     * @brief Constructs a new instance of type FitzHughNagumoModelBase with 0 neurons and the passed values for all parameters.
     * @param h See NeuronModel(...)
     * @param activity_input See NeuronModel(...)
     * @param fired_status_communicator See NeuronModel(...)
     * @param params The model parameters
     */
    FitzHughNagumoModelBase(
        unsigned int h,
     ...
     */
    [[nodiscard]] const models::fitzhughnagumo::Parameters<RelearnTypes::activity_type>& get_model_parameters() const noexcept {
        return parameters;
    }
...
protected:
    ...
    LazySyncedArray<activity_type> w; // recovery variable

    models::fitzhughnagumo::Parameters<RelearnTypes::activity_type> parameters;
};
