 ...
 *      Holds everything that is common to the CPU and GPU Poisson models; PoissonModelCPU and
 *      PoissonModelGPU add the parts that differ (update_activity(_benchmark), and the GPU side
 *      additionally registers a random key and re-initializes the neurons on init/create_neurons).
 */
class PoissonModelBase : public NeuronModel {
public:
    /**
     * @brief Constructs a new instance of type PoissonModelBase with 0 neurons and the passed values for all parameters.
     ...
     */
    [[nodiscard]] const models::poisson::Parameters<RelearnTypes::activity_type, unsigned int>& get_model_parameters() const noexcept {
        return parameters;
    }
...
protected:
    ...
    LazySyncedArray<unsigned int> refractory_time; // refractory time

    models::poisson::Parameters<RelearnTypes::activity_type, unsigned int> parameters{};
};
