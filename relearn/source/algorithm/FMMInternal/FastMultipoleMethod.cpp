...
FastMultipoleMethod::find_target_neurons(const number_neurons_type number_neurons) {
    ...
    RelearnException::check(gaussian_ptr != nullptr,
                            "FastMultipoleMethod::find_target_neurons: kernel was not Gaussian");

    if (total_number_dendrites_ex > 0) {
        FastMultipoleMethodBase::make_creation_request_for(gaussian_ptr->get_sigma(), node_cache, root,
                                                           local_branch_nodes, branch_level, ElementType::Axon,
                                                           SignalType::Excitatory, Constants::unpacking + 1,
                                                           synapse_creation_requests_outgoing);
    }
    if (total_number_dendrites_in > 0) {
        FastMultipoleMethodBase::make_creation_request_for(gaussian_ptr->get_sigma(), node_cache, root,
                                                           local_branch_nodes, branch_level, ElementType::Axon,
                                                           SignalType::Inhibitory, Constants::unpacking + 1,
                                                           synapse_creation_requests_outgoing);
    }
...
FastMultipoleMethod::process_requests(
...
FastMultipoleMethod::process_responses(const RelearnTypes::comm_map_creation<SynapseCreationRequest>& creation_requests,
