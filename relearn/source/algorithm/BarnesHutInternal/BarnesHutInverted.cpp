...
BarnesHutInverted::find_target_neurons(const number_neurons_type number_neurons) {
    ...
    const auto& vacant_excitatory_dendrites = synaptic_elements->get_vacant_elements(
        SynapticElementType::DendriteExcitatory);
    const auto& vacant_inhibitory_dendrites = synaptic_elements->get_vacant_elements(
...
#pragma omp parallel for default(none) shared(probability_kernel, node_cache, root, my_rank, number_neurons, disable_flags, vacant_excitatory_dendrites, vacant_inhibitory_dendrites, synapse_creation_requests_outgoing)
    for (auto neuron_id = number_neurons_type{ 0 }; neuron_id < number_neurons; ++neuron_id) { // NOLINT(openmp-exception-escape) - only throwable calls are RelearnException::check on already-validated algorithm invariants
        ...
        const auto& excitatory_requests = BarnesHutBase<BarnesHutInvertedCell>::find_target_neurons(probability_kernel,
                                                                                                    node_cache,
                                                                                                    { my_rank, id },
                                                                                                    dendrite_position,
...
#pragma omp critical(BHIrequests)
        ...
        const auto& inhibitory_requests = BarnesHutBase<BarnesHutInvertedCell>::find_target_neurons(probability_kernel,
                                                                                                    node_cache,
                                                                                                    { my_rank, id },
                                                                                                    dendrite_position,
...
BarnesHutInverted::find_target_neurons_for_combined_algorithms(const std::vector<NeuronID>& neuron_ids) {
    const auto& disable_flags = extra_infos->get_disable_flags();
    const auto number_ranks = mpiPP::MPIInfo::get_number_ranks();
    const auto my_rank = mpiPP::MPIInfo::get_my_rank();
...
BarnesHutInverted::process_responses(const RelearnTypes::comm_map_creation<SynapseCreationRequest>& creation_requests,
