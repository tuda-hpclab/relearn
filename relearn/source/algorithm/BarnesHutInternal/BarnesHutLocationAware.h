 ...
 */
class BarnesHutLocationAware : public ForwardAlgorithm<DistantNeuronRequest, DistantNeuronResponse>, private OctreeAlgorithm<BarnesHutCell> {
public:
    ...
    virtual ~BarnesHutLocationAware() = default;
    ...
    void record_memory_footprint(const std::unique_ptr<utility::MemoryFootprint>& footprint) override {
        const auto my_footprint = sizeof(*this) - sizeof(ForwardAlgorithm<DistantNeuronRequest, DistantNeuronResponse>);
        footprint->emplace("BarnesHutLocationAware", my_footprint);

        ForwardAlgorithm<DistantNeuronRequest, DistantNeuronResponse>::record_memory_footprint(footprint);
    }
   ...
   [[nodiscard]] AlgorithmEnum get_algorithm_type() const override {
     ...
     */
    [[nodiscard]] std::pair<RelearnTypes::comm_map_creation<DistantNeuronResponse>, std::pair<PlasticLocalSynapses, PlasticDistantInSynapses>>
