...
#include "neurons/enums/BackgroundActivityCalculatorType.h"
...
static void simulate(Simulation simulation, const RelearnTypes::step_type simulation_steps, const bool interactive) {
...
int main(int argc, char** argv) {
    ...
    auto chosen_fired_status_communicator_type = FiredStatusCommunicatorType::Map;
    ...
    auto chosen_space_filling_curve_type = SpaceFillingCurveType::Morton;
    const auto cli_parse_space_filling_curve_type = std::map<std::string, SpaceFillingCurveType>{
        { "morton", SpaceFillingCurveType::Morton },
        { "hilbert", SpaceFillingCurveType::Hilbert },
    };
    ...
    auto fraction_excitatory_neurons = double{ 1.0 };
    ...
    auto accept_criterion = double{ Constants::bh_default_theta };
    ...
    auto base_background_activity = double{ ConstantActivityInput::default_constant_activity };
    ...
    auto background_activity_mean = double{ NormalActivityInput::default_mean_activity };
    ...
    auto background_activity_stddev = double{ NormalActivityInput::default_stddev_activity };
    ...
    auto activity_input_str = std::string{ "scale(synaptic_equally_weighted, linear)" };
...
size_t -> literal)help");
    ...
    auto synapse_conductance = double{ 0.03 };
    ...
    auto calcium_decay = double{ CalciumCalculator::default_tau_C };
    ...
    auto target_calcium_decay_amount = double{ 0.0 };
    ...
    auto beta = double{ CalciumCalculator::default_beta };
    ...
    auto synaptic_elements_init_ub = double{ 0.0 };
    ...
    auto nu_dend_ex = double{ SynapticElements::default_nu };
    ...
    auto growth_rate_decay = double{ 10000.0 };
    ...
    auto min_calcium_axons = double{ SynapticElements::default_eta_Axons };
    ...
    auto min_calcium_excitatory_dendrites = double{ SynapticElements::default_eta_Dendrites_exc };
    ...
    auto min_calcium_inhibitory_dendrites = double{ SynapticElements::default_eta_Dendrites_inh };
    ...
    app.add_option("--monitor-flush-step", Config::flush_group_monitor_step, "The steps when to flush the neuron monitors. Must be > 0");

    auto percentage_initial_fired_neurons = double{ 0.0 };
    ...
    RelearnException::check(Config::flush_group_monitor_step > 0, "The step for flushing the neuron monitors must be > 0.");
    ...
    auto construct_fired_status_communicator = [&]() -> std::shared_ptr<FiredStatusCommunicator> {
        if (chosen_fired_status_communicator_type == FiredStatusCommunicatorType::Map) {
            return std::make_shared<FiredStatusCommunicationMap>(mpiPP::MPIInfo::get_number_ranks());
        }
        ...
        return std::make_shared<FiredStatusApproximator>(mpiPP::MPIInfo::get_number_ranks());
    };
    auto fired_status_comm = construct_fired_status_communicator();

    auto parse_context = ActivityInputParseContext{
        .background_base = base_background_activity,
        .background_mean = background_activity_mean,
        .background_stddev = background_activity_stddev,

        .communicator = fired_status_comm,

        .linear = [synapse_conductance](const double val) { return synapse_conductance * val; },
        .logarithmic = [input_scale, synapse_conductance](const double val) { return input_scale * std::log10(synapse_conductance * val + 1.0); },
        .hyperbolic_tangent = [input_scale, synapse_conductance](const double val) { return input_scale * std::tanh(synapse_conductance * val); },
    ...
    auto construct_neuron_model = [&]() -> std::unique_ptr<NeuronModel> {
        ...
        if (chosen_neuron_model == NeuronModelType::Poisson) {
            return std::make_unique<PoissonModel>(h, std::move(combined_activity_input), std::move(fired_status_comm),
                                                  poisson::Parameters<double, unsigned int>{});
        }
        ...
        if (chosen_neuron_model == NeuronModelType::FitzHughNagumo) {
            return std::make_unique<FitzHughNagumoModel>(h, std::move(combined_activity_input), std::move(fired_status_comm),
                                                         fitzhughnagumo::Parameters<double>{});
        }
    ...
    auto construct_growth_rate_calculator = [&](const double growth_rate) -> std::shared_ptr<GrowthrateCalculator> {
    ...
    auto construct_synaptic_elements = [&] {
        auto construct_axons = [&]() -> std::shared_ptr<Axons> {
            ...
            if (chosen_axons == AxonsType::MultiPosition) {
                ...
                axons->set_bouton_positions(std::move(positions));
        ...
        if (*opt_synaptic_elements_file) {
            const auto& [min_calcium_axons_calculator, nu_axons_calculator, vacant_retract_ratio_axons_calculator, min_elements_axons, max_elements_axons_calculator,
                         min_calcium_den_exc_calculator, nu_den_exc_calculator, vacant_retract_ratio_den_exc_calculator, min_elements_den_exc_calculator, max_elements_den_exc_calculator,
                         min_calcium_den_inh_calculator, nu_den_inh_calculator, vacant_retract_ratio_den_inh_calculator, min_elements_den_inh_calculator, max_elements_den_inh_calculator]
                = SynapticElementsIO::load_function_from_file(synaptic_elements_file, my_rank, subdomain->get_local_group_translator());

            synaptic_elements->set_minimum_calcium_calculator(min_calcium_axons_calculator, SynapticElementType::Axon);
            synaptic_elements->set_minimum_calcium_calculator(min_calcium_den_exc_calculator, SynapticElementType::DendriteExcitatory);
            synaptic_elements->set_minimum_calcium_calculator(min_calcium_den_inh_calculator, SynapticElementType::DendriteInhibitory);

            synaptic_elements->set_vacant_retract_ratio_calculator(vacant_retract_ratio_axons_calculator, SynapticElementType::Axon);
            synaptic_elements->set_vacant_retract_ratio_calculator(vacant_retract_ratio_den_exc_calculator, SynapticElementType::DendriteExcitatory);
            synaptic_elements->set_vacant_retract_ratio_calculator(vacant_retract_ratio_den_inh_calculator, SynapticElementType::DendriteInhibitory);

            return synaptic_elements;
        }

        auto construct_const_calculator = [](const double constant) {
        ...
        auto construct_grown_element_calculator = [synaptic_elements_init_lb, synaptic_elements_init_ub](const RelearnTypes::number_neurons_type /*neuron_id*/) {
            ...
            return RandomHolder::get_random_uniform_double(RandomHolderKey::SynapticElements, synaptic_elements_init_lb, synaptic_elements_init_ub);
        };
