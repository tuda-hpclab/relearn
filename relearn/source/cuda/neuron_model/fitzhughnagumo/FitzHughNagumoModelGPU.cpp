...
using models::FitzHughNagumoModelGPU;

void FitzHughNagumoModelGPU::update_activity() {
...
void FitzHughNagumoModelGPU::update_activity_benchmark() {
    const number_neurons_type num_neurons = get_number_neurons();
...
void FitzHughNagumoModelGPU::record_memory_footprint(const std::unique_ptr<utility::MemoryFootprint>& footprint) {
    FitzHughNagumoModelBase::record_memory_footprint(footprint);
    footprint->emplace("FitzHughNagumoModel GPU", w.get_memory_footprint());
}
