...
using models::AEIFModelGPU;

void AEIFModelGPU::update_activity() {
...
void AEIFModelGPU::update_activity_benchmark() {
    const number_neurons_type num_neurons = get_number_neurons();
...
void AEIFModelGPU::record_memory_footprint(const std::unique_ptr<utility::MemoryFootprint>& footprint) {
    AEIFModelBase::record_memory_footprint(footprint);
    footprint->emplace("AEIFModel GPU", w.get_memory_footprint());
}
