...
void NeuronsExtraInfoAdapter::enable_all(const std::shared_ptr<NeuronsExtraInfo>& extra_info) {
    ...
    const auto ids_to_enable = NeuronID::range(disable_flags.size()) | ranges::views::filter(utility::equal_to(UpdateStatus::Disabled), utility::lookup(disable_flags, &NeuronID::get_neuron_id)) | ranges::to_vector;
...
void NeuronsExtraInfoAdapter::disable_all(const std::shared_ptr<NeuronsExtraInfo>& extra_info) {
    ...
    const auto ids_to_disable = NeuronID::range(disable_flags.size()) | ranges::views::filter(utility::equal_to(UpdateStatus::Enabled), utility::lookup(disable_flags, &NeuronID::get_neuron_id)) | ranges::to_vector;
